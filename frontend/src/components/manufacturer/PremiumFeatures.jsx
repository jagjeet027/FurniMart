  ...
  const features = [
    {
      ...
      description:
        "Transform your business with cutting-edge AI solutions and smart payment processing",
      icon: TrendingUp,
    ...
    },
    {
      ...
      description:
        "Access top-tier professionals with AI-driven recruitment and HR automation",
      icon: Users,
    ...
    },
    {
      ...
      description:
        "Expand to 190+ countries with intelligent logistics and localized solutions",
      icon: Globe,
    ...
    },
    {
      ...
      description:
        "Lead your industry with ML-powered insights and dynamic market analysis",
      icon: ShoppingBag,
      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          if (manufacturerData.contact?.email) {
            setFormData((prev) => ({
              ...prev,
              email: manufacturerData.contact.email,
            }));
          }
          if (manufacturerData.contact?.phone) {
            setFormData((prev) => ({
              ...prev,
              phone: manufacturerData.contact.phone,
            }));
          }
        } else {
          throw new Error(
            "Manufacturer profile not found. Please complete your manufacturer registration first."
          );
        }
      } catch (error) {
        ...
        if (error.response?.status === 404) {
          ...
          setError(
            error.response?.data?.message ||
              error.message ||
              "Failed to load manufacturer data"
          );
        }
      } finally {
        setLoading(false);
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const verificationData = await api.post(
        ...
        {plans.map((plan, index) => (
          ...
          >
            ...
            <p className="text-4xl font-bold mb-2">
              {plan.title === "Enterprise Partnership"
                ? plan.price
                : `₹${plan.price}`}
            </p>
